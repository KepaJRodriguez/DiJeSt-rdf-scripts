...
with open ('DiJeSt_personalities.csv', 'r') as csvfile:
	...
	for row in reader:
		if row['entityID'] in entities_links:
			...
			#graph.add((URIRef(person_uri), RDF['type'], schema['Person']))
			#graph.add((URIRef(person_uri), RDF['type'], eaccpf['Person']))
			#graph.add((URIRef(person_uri), RDF['type'], dbo['Person']))
			graph.add((URIRef(person_uri), RDF['type'], djo['Person']))
			#graph.add((URIRef(person_uri), RDF['type'], owl['Thing']))
			...
			if len(row['birthPlace']) > 0:
				...
				if len(row['birthPlaceKima']) > 0:
					graph.add((birthPlace_node, schema['url'], URIRef(row['birthPlaceKima'])))				
				graph.add((URIRef(person_uri), schema['birthPlace'], birthPlace_node))
			...
			if len(row['associatedPlaces']) > 0:
				...
				if len(associatedPlaces) > 0:
					...
					for assp in associatedPlaces:
						counterAP += 1
						assocPlaceNodeName = 'associated-place-' + row['entityID'] + '-' + str(counterAP)
						assocPlaceNode = BNode(assocPlaceNodeName)
						#graph.add((assocPlaceNode, RDF['type'], schema['Place']))
						graph.add((assocPlaceNode, RDF['type'], djo['Place']))
						place_name = assp.keys()[0]
						graph.add((assocPlaceNode, eaccpf['hasPlace'], Literal(place_name)))
			...
			if len(row['firstNameAra']) > 0 and len(row['lastNameAra']) > 0:
				label_name_heb = row['firstNameAra'] + ' ' + row['lastNameAra']
